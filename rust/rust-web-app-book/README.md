...
- 参照を扱おうとするとでてくるArcとか
- 非同期処理
- コンパイル時間短縮のためにcrate分割が推奨されているが、依存関係の増減があるたびに`Cargo.toml`を手書きしないといけないのが若干面倒
- DIが若干面倒かも
    - Goならinterface使うだけなので楽

## fzf-makeの開発に活かせそうな学び
- `fzf_make_main.rs`みたいなファイルを作らずとも`mod.rs`に書いていくのでもいいかも。(`adapter/src/redis/mod.rs`がそんな感じだった)
